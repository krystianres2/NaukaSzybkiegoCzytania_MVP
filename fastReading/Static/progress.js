$(document).ready(function () {
  ...
  function displayData(list, lastList) {
    ...
    let difference = calculateWpmDifference(groupDataByDay(lastList));
    displayDifference($("#last_progress"), difference, true);
    ...
    difference = calculateWpmDifference(groupDataByMonth(list));
    displayDifference($("#progress"), difference);
  ...
  function displayDifference(selector, difference) {
    if (difference > 0) {
      selector.html(
        `Różnica: <span style="color: green;">${difference}</span>`
      );
    } else if (difference < 0) {
      selector.html(
        `Różnica: <span style="color: red;">-${Math.abs(difference)}</span>`
      );
    } else {
      selector.text(`Różnica: ${difference}`);
    }
  }
